...
func vmwareStorageType() {
	...
	if provider == "VSphere" {
		...
		storageType, _ = utils.GetDatastoreStorageType(vCenterURL, username, password, dataCenter)

		color.HiCyan("VMware Datastore Storage Provider\t\t\tCHECKED")

	} else {
		color.HiCyan("VMware Datastore Storage Provider\t\t\tSKIPPED")
	}
	...
	for fileScanner.Scan() {
		...
		output.WriteString(vmwareStorageTypeProcess(line))
	}
...
func vmwareStorageTypeProcess(line string) string {

	if strings.HasPrefix(line, "Datastore Storage Provider") {
		if storageType != "" && provider == "VSphere" {
